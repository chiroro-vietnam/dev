...
<body>
<form action="<?php echo base_url(); ?>request/kaigisho-dt/order_c/input" method="post" id="orderc_input">
  ...
  <tr>
	...
	<table width="100%" border="1" cellspacing="0" cellpadding="5">
	  ...
	  <tr>
		...
		<td><font color="#0099ff">「Cコース」●初15,000円／月5,000円</font>
		<input name="plan_name" type="hidden" size="40" value="「Cコース」●初15,000円／月5,000円" />
		</td>
	  </tr>
	  <tr>
		<td width="20%" class="col3">ディスク容量</td>
		<td>60GB
		<input name="disk_space" type="hidden" size="40" value="60GB" />
		</td>
	  </tr>
	  <tr>
		<td width="20%" class="col3">保証メモリ</td>
		<td>1GB
		<input name="guarantee_memory" type="hidden" size="40" value="1GB" />
		</td>
	  </tr>
	  <tr>
		<td width="20%" class="col3">最大メモリ</td>
		<td>2GB <input name="max_memory" type="hidden" size="40" value="2GB" /></td>
	  </tr>
	  <tr>
		<td width="20%" class="col3">プロセス数上限</td>
		<td>1024 <input name="limit_memory" type="hidden" size="40" value="1024" /></td>
	  </tr>
	  <tr>
		...
		<td>
			...
			<?php 
				$arr_manager = array(                   
					'plesk10' => 'Plesk(10ドメイン版)●初5,000円／月1,600円'                   
				); 
				foreach($arr_manager as $manager => $value)
				{                 
				   echo '<option value="'. $value .'"';
				   if(!empty($data_old['manager']) && $data_old['manager'] == $value){
						echo ' selected="selected"';
				   }
					echo '>'. $value .'</option>';
				}
	  ...
	  <tr>
		...
		<td>
			...
			<?php 
				$arr_ssl = array(
					'AlphaSSL_01year' => '「アルファSSL1ヶ年」＋設定●年10,000円'                   
				); 
				foreach($arr_ssl as $ssl => $value)
				{
				   echo '<option value="'. $value .'"';
				   if(!empty($data_old['ssl']) && $data_old['ssl'] == $value){
						echo ' selected="selected"';
					}
					echo '>'. $value .'</option>';
				}
		...
		<td>
			...
			<?php 
				$arr_IP_address = array(                   
					'02' => '2個●初2,000円／月2,000円'                   
				); 
				foreach($arr_IP_address as $IP_address => $value)
				{
				   echo '<option value="'. $value .'"';
				   if(!empty($data_old['IP_address']) && $data_old['IP_address'] == $value){
						echo ' selected="selected"';
					}
					echo '>'. $value .'</option>';
				}
	  ...
	  <tr>
		<td width="20%" class="col3">PHP</td>        
		  <td>
			<select name="PHP">
				<option value="PHP5.2.x" selected="selected">PHP5.2.x</option>
			<?php 
				$arr_PHP = array(
					'PHP5.3.x' => 'PHP5.3.x',
					'PHP5.4.x' => 'PHP5.4.x',
	  ...
	  <tr>
		...
		<td>
			<select name="mysql">
				<option value="MySQL5.0.x" selected="selected">MySQL5.0.x</option>
			<?php 
				$arr_mysql = array(
					'MySQL5.1.x' => 'MySQL5.1.x',
					'MySQL5.5.x' => 'MySQL5.5.x',
					'MySQL5.6.x' => 'MySQL5.6.x',
					'no_thanks' => '不要'
				); 
				foreach($arr_mysql as $mysql => $value)
				{
				   echo '<option value="'. $value .'"';
				   if(!empty($data_old['mysql']) && $data_old['mysql'] == $value){
						echo ' selected="selected"';
					}
					echo '>'. $value .'</option>';
				}
	  ...
	  <tr>
		<td width="20%" class="col3">PostgreSQL</td>
		  <td>
			...
			<?php 
				$arr_postgresql = array(
					'PostgreSQL8.2' => 'PostgreSQL8.2',
					'PostgreSQL8.3.x' => 'PostgreSQL8.3.x',
					'PostgreSQL9.0.x' => 'PostgreSQL9.0.x',
					'PostgreSQL9.1.x' => 'PostgreSQL9.1.x',
					'PostgreSQL9.2.x' => 'PostgreSQL9.2.x',
					'PostgreSQL9.3.x' => 'PostgreSQL9.3.x',
				); 
				foreach($arr_postgresql as $postgresql => $value)
				{                 
				   echo '<option value="'. $value .'"';
				   if(!empty($data_old['postgresql']) && $data_old['postgresql'] == $value){
						echo ' selected="selected"';
				   }
					echo '>'. $value .'</option>';
				}
